...
export default function PickupReminders({ suppliers }: { suppliers: Supplier[] }) {
  ...
  return (
    <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/60 rounded-2xl p-6 shadow-sm animate-in fade-in duration-300">
      <div className="flex items-center gap-3 mb-4">
        <div className="p-2.5 bg-amber-500 text-white rounded-xl shadow-md shadow-amber-500/20 animate-pulse">
        ...
        <div>
          <h3 className="font-bold text-slate-800 text-base md:text-lg">Pengingat Jadwal Ambilan Besok (H-1)</h3>
          <p className="text-xs text-slate-500">Konfirmasikan jadwal pengambilan barang ke lapak-lapak berikut agar rute armada terencana.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {activeReminders.map(supplier => (
          <div
            key={supplier.id}
            className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-amber-200 hover:border-amber-300 hover:shadow-md transition-all flex flex-col justify-between gap-4"
          >
            <div className="flex justify-between items-start">
              <div>
                <h4 className="font-bold text-slate-800 text-sm md:text-base">{supplier.nama}</h4>
                ...
                <div className="mt-1 text-xs text-amber-700 bg-amber-100/50 px-2.5 py-0.5 rounded-full inline-block font-semibold">
            ...
            <div className="flex gap-2">
              {supplier.kontak_wa ? (
                <a
                  ...
                  className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2 px-3 rounded-lg text-xs flex items-center justify-center gap-1.5 shadow-sm transition-all hover:scale-[1.02]"
                >
                ...
                <div className="flex-1 bg-slate-100 text-slate-500 py-2 px-3 rounded-lg text-xs flex items-center justify-center gap-1.5 font-medium">
                  WA Belum Terdaftar
                </div>
              )}
